...
class ImageProcessor():
    def __init__(self, map_builder_callback):
        ...
        self.transform = A.Compose([A.Resize(256,256),TT.ToTensor()])
        self.transform_depth = A.Compose([A.Resize(512,512),TT.ToTensor()])
        self.counter = 0
        self.lock = Lock()
    ...
    def subscribe(self, msg):
        ...
        if self.last_update_time != None and time.time() - self.last_update_time > 3:
            ...
            #image = input.reshape((3, msg[1][0], msg[1][1]))
            image = input.reshape((msg[1][0], msg[1][1], 3))
            #tensor = torch.tensor(image).float()
            #tensor2 = torch.tensor(image).float()
            tensor = self.transform(image = image, bboxes = None, labels=None)["image"]
            tensor2 = self.transform_depth(image = image, bboxes = None, labels = None)["image"]
            ...
            if(detection_predict[0]['boxes'].size()[0]!=0):
                ...
                image = torchvision.transforms.ToPILImage()(tensor).resize((128,128))
